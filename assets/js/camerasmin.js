...
function OnMlistcam(evt) {
	if(evt.data.search("6606672e-57f8-47d1-8002-5fe59d34c1d8") != -1){		
		listCameras(evt.data)		
	}
}
function listCameras(devices){
	document.getElementById("cameras").innerHTML = "";
	// document.getElementById("deviceSwitch").innerHTMLcam = "";
	// document.getElementById("deviceUnknown").innerHTMLcam = "";
	// document.getElementById("deviceContact").innerHTMLcam = "";
	// document.getElementById("deviceKeyfob").innerHTMLcam = "";
	// document.getElementById("deviceCamera").innerHTMLcam = "";
	// document.getElementById("deviceThermostat").innerHTMLcam = "";
	// document.getElementById("deviceFanControl").innerHTMLcam = "";
			...
			var device = Devices[i].split(",");
				for (var x = 0; x < device.length; x++) {
					...
					// } else if(window.location.href.indexOf("cameras.html") > -1) {
						...
						+ ' <div class="tooltip"><i ondblclick="window.location=' + camsetInfoLink + ' class="fa fa-cog"></i> <span class="tooltiptext">Settings</span> </div> <br>'
					// }
					+ devPresence 
					+ '<div id="HVAC' + devID + '">' + devThermState + devHeatSetPoint + devCoolSetPoint + '</div>'
					+ '<div id="Motion' + devID + '">' + devMotionState + '</div>';
					deviceTypeCam(devType,devID,devState,devContact);
...
function deviceTypeCam(device,ID,state,devContact){
						...
						if (device.search("Thermostat") == -1){
							if (device.search("Fan Control") == -1){
								if (device.search("Dimmer") == -1){
									// document.getElementById("deviceUnknown").innerHTMLcam = document.getElementById("deviceUnknown").innerHTMLcam + HTMLcam + "<P>";
									HTMLcam = "";
								}
							}
						}
	...
	if (device.search("Camera") != -1){
		//writeToScreen(ID);
		document.getElementById("cameras").innerHTML = document.getElementById("cameras").innerHTML + '<article><a href="https://svp.irisbylowes.com/preview/' + placeID + '/' + ID + '"  class="image fit"><img src="https://svp.irisbylowes.com/preview/' + placeID + '/' + ID + '" ></a><center>' + HTMLcam + "</center></article>";
		HTMLcam = "";
	}else{
		HTMLcam = "";
	}
