...
function onMbatterylevel(evt) {
	if(evt.data.search("6606672e-57f8-47d1-8002-5fe59d34c1d8") != -1){
		listbatterylevel(evt.data)			
	}
}
function listbatterylevel(devices){
	...
	HTMLtable = '<table class="tablesorter"><thead><tr><th>Battery Icon</th><th><i class="fa fa-sort"></i> Device Name</th><th><i class="fa fa-sort"></i> Battery Level</th></tr></thead><tbody>';
		...
		for (var i = 0; i < Devices.length; i++) {
			var device = Devices[i].split(",");
				for (var x = 0; x < device.length; x++) {
					...
					HTML = HTML + '<tr><td>' + devBatteryNum + '</td>'
					+ '<td>' + devName + '</td>'
					+ '<td>' + devBattery + '</td></tr>';
					deviceType(devBatteryNum,devBattery,devType,devID,devState,devContact);
...
function deviceType(batteryicon,batterylevel,device,ID,state,devContact){
