...
function onMtemperaturelevel(evt) {
	if(evt.data.search("6606672e-57f8-47d1-8002-5fe59d34c1d8") != -1){
		listtemperaturelevel(evt.data)			
	}
}
function listtemperaturelevel(devices){
	...
	HTMLtable = '<table class="tablesorter"><thead><tr><th ><i class="fa fa-sort"></i> Temperature</th><th><i class="fa fa-sort"></i> Device Name</th></tr></thead><tbody>';
		...
		for (var i = 0; i < Devices.length; i++) {
			var device = Devices[i].split(",");
				for (var x = 0; x < device.length; x++) {
					...
					HTML = HTML + '<tr id="' + i + '"><td class="sort-card" >' + devTemp + '</td>'
					+ '<td>' + devName + '</td></tr>';
					deviceType(devTemp,devType,devID,devState,devContact);
...
function deviceType(temperaturelevel,device,ID,state,devContact){
