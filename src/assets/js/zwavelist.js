...
function listdeviceslevel(devices){
	...
	HTMLtable = '<table class="tablesorter"><thead><tr><th><i class="fa fa-sort"></i> Name</th><th><i class="fa fa-sort"></i> Protocol ID</th><th><i class="fa fa-sort"></i> Protocol ID</th></tr></thead><tbody>';
		...
		for (var i = 0; i < Devices.length; i++) {
			var device = Devices[i].split(",");
				for (var x = 0; x < device.length; x++) {
					...
					if(device[x].search("devadv:protocolid") != -1){
					protocolid = device[x].replace(/'"'/g,'').replace('"devadv:protocolid":"',"").replace('"',"") + "<br>";
					...
					HTML = HTML + '<tr><td>' + devName + '</td>' 
					+ '<td>' + protocolid + '</td>'
					+ '<td> ' + devProtocol + '</td></tr>';
