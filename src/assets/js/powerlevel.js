...
function onMpowerlevel(evt) {
	if(evt.data.search("6606672e-57f8-47d1-8002-5fe59d34c1d8") != -1){
		listpowerlevel(evt.data)			
	}
}
function listpowerlevel(devices){
	...
	HTMLtable = '<table class="tablesorter"><thead><tr><th><i class="fa fa-sort"></i> Device</th><th><i class="fa fa-sort"></i> Power Useage</th></tr></thead><tbody>';
			...
			var device = Devices[i].split(",");
				for (var x = 0; x < device.length; x++) {
					...
					if(device[x].search('pow:instantaneous') != -1) {
						...
						devTotalPower = devTotalPower + parseInt(devPower);
					}
					...
					HTML = HTML + '<tr><td><i class="fa fa-plug"></i> ' + devName + '</td>'
					+ '<td><i class="fa fa-bolt"></i> ' + devPower + ' Watts</td></tr>';
					deviceType(devPower,devType,devID,devState,devContact);
	...
	document.getElementById("tbody").innerHTML = HTMLtable + '</tbody><tfoot><tr><td></td><td>Total Wattage: ' + devTotalPower + ' </td></tr></tfoot></table>';
...
function deviceType(powerlevel,device,ID,state,devContact){
